...
int main()
{
	...
	while (window.isOpen())
	{
		...
		// Ternary operator when an if would do! This performs a console write operation every frame for no reason
		cout << ((result != 0) ? ("Collision") : "") << endl; 

		// Why not do this ^^^ down here vvv ?
		if (result){
			player.colliding(npc);
		}
		else {
