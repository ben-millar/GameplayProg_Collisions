...
AnimatedSprite::AnimatedSprite(const sf::Texture& t) : Sprite(t), m_current_frame(0), m_time(seconds(0.25f)) {}
...
void AnimatedSprite::update(){
	if (m_clock.getElapsedTime() > m_time) {
		if (m_frames.size() > static_cast<unsigned>(m_current_frame + 1))
		{
			m_current_frame++;
		}
		else {
